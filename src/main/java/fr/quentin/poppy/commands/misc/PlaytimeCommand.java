...
public class PlaytimeCommand extends SafeCommand {
    ...
    protected boolean execute(@NonNull CommandSender sender, @NonNull Command command, @NonNull String label, String @NonNull [] args) {
        ...
        if (!player.hasPermission("poppy.playtime.admin")) {
        ...
        if (targetUuid == null) {
            player.sendMessage(messages.get("playtime.admin-target-not-found", "player", args[0]));
            return true;
        }
        ...
        sendPlaytime(player, targetUuid, target.getFirstPlayed(), targetName);
        return true;
    }
